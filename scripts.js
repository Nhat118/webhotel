document.addEventListener("DOMContentLoaded", function () {
    ...
    if (roomSearchForm) {
      roomSearchForm.addEventListener("submit", function (e) {
        ...
        const roomType = document.getElementById("roomTypeInput").value.toLowerCase().trim();
    ...
    if (bookingFormModal) {
      bookingFormModal.addEventListener('submit', function (e) {
        ...
        if (!checkin || !checkout || !guests || !customerName || !customerPhone || !customerEmail) {
        ...
        const roomCard = Array.from(document.querySelectorAll('.card')).find(card => 
          card.querySelector('h4').textContent.trim().toLowerCase() === roomName.trim().toLowerCase()
        );
        if (roomCard) {
    ...
    roomCards.forEach(card => {
      ...
      const isBooked = bookingHistory.some(booking => 
        booking.roomName.trim().toLowerCase() === roomName
      );
